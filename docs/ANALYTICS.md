...
| `feature_view` | Sentinela `IntersectionObserver` (`SectionViewTracker`, mesmo padrão do `pricing_view`), reaproveitado em ambas as páginas nas seções de demonstração do produto (clientes/avaliações + agenda/ciclos), Cronia e desktop+mobile. | `feature_id` | `{ event: "feature_view", feature_id: "home_product_showcase" }` — valores usados: `home_product_showcase`, `home_devices`, `lp_ads_product_showcase`, `lp_ads_assistant`, `lp_ads_devices` |
