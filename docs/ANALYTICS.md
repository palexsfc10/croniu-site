...
| `feature_view` | Sentinela `IntersectionObserver` (`SectionViewTracker`, mesmo padrão do `pricing_view`), reaproveitado em ambas as páginas: 4 seções de prova na landing page de Google Ads (visual do produto, Cronia, gestão agenda/alunos/ciclos/financeiro e desktop+mobile) e 2 na home (gestão agenda/clientes/ciclos/financeiro e desktop+mobile). | `feature_id` | `{ event: "feature_view", feature_id: "lp_ads_visual" }` — valores usados: `lp_ads_visual`, `lp_ads_assistant`, `lp_ads_management`, `lp_ads_devices`, `home_management`, `home_devices` |
